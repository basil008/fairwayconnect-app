/**
 * DANGER: Drops and recreates ALL tables. CLI-only — this is intentionally
 * NOT an HTTP endpoint any more.
 *
 * Usage (local):   npx tsx scripts/recreate-schema.ts --yes
 * Usage (prod):    fly ssh console -a fairwayconnect-live, then run with --yes
 */
import { getDb } from '../src/lib/db';

if (!process.argv.includes('--yes')) {
  console.error('Refusing to run without --yes. THIS DESTROYS ALL DATA.');
  process.exit(1);
}
