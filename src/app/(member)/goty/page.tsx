            ...
            <table className="w-full border-collapse text-sm">
              <thead>
                <tr className="bg-fairway-900 text-white">
                  <th className="px-2 py-3 text-left text-xs font-bold w-8 sticky left-0 bg-fairway-900 z-10">#</th>
                  <th className="px-2 py-3 text-left text-xs font-bold min-w-[140px] sticky left-8 bg-fairway-900 z-10">Player</th>
                  <th className="px-3 py-3 text-center text-xs font-bold w-16 bg-yellow-600 sticky left-[188px] z-10">
                ...
                {standings.map((entry, i) => {
                  ...
                  return (
                    <tr key={entry.member_id} className={`border-b border-gray-100 ${isTop3 ? 'bg-yellow-50' : i % 2 === 0 ? 'bg-white' : 'bg-gray-50'}`}>
                      <td className={`px-2 py-2.5 text-center font-bold text-gray-500 sticky left-0 z-10 ${isTop3 ? 'bg-yellow-50' : i % 2 === 0 ? 'bg-white' : 'bg-gray-50'}`}>
                        {getMedal(entry.position)}
                      </td>
                      <td className={`px-2 py-2.5 sticky left-8 z-10 ${isTop3 ? 'bg-yellow-50' : i % 2 === 0 ? 'bg-white' : 'bg-gray-50'}`}>
                        <span className={`font-semibold ${isTop3 ? 'text-fairway-900' : 'text-gray-800'}`}>
                          {shortName(entry.name)}
                        </span>
                      </td>
                      <td className={`px-3 py-2.5 text-center font-bold text-lg ${isTop3 ? 'text-fairway-900' : 'text-gray-800'} sticky left-[188px] z-10`}
                        style={isTop3 ? { background: 'rgba(234, 179, 8, 0.15)' } : { background: i % 2 === 0 ? 'white' : '#f9fafb' }}>
                        {entry.total_points}
                      </td>
