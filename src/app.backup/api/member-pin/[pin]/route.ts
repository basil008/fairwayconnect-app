import { NextRequest, NextResponse } from 'next/server';
import { getDb } from '@/lib/db';
import { seedDatabase } from '@/lib/seed';

export async function GET(_req: NextRequest, { params }: { params: Promise<{ pin: string }> }) {
  await seedDatabase();
  const db = getDb();
  const { pin } = await params;

  if (!pin || pin.length !== 4) {
    return NextResponse.json({ error: 'Invalid PIN' }, { status: 400 });
  }

  // Add member_pin column if not exists
  try { 
    await db.execute("ALTER TABLE members ADD COLUMN member_pin TEXT");
  } catch { 
    /* already exists */ 
  }

  // Look up member by PIN
  const memberResult = await db.execute({ 
    sql: 'SELECT id, name, handicap, member_type FROM members WHERE member_pin = ?',
    args: [pin]
  });
  const member = memberResult.rows[0] as any;

  if (!member) {
    return NextResponse.json({ error: 'Member not found' }, { status: 404 });
  }

  return NextResponse.json({ id: member.id, name: member.name, handicap: member.handicap, member_type: member.member_type });
}
